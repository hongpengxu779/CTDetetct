...
class MedianFilterDialog(QtWidgets.QDialog):
    """中值滤波对话框，用于预览和配置中值滤波参数"""
    ...
    def init_ui(self):
        ...
        # 参数设置区域
        param_group = QtWidgets.QGroupBox("卷积核参数")
        param_layout = QtWidgets.QGridLayout(param_group)
        
        # X方向半径
        param_layout.addWidget(QtWidgets.QLabel("X方向半径:"), 0, 0)
        self.x_spinbox = QtWidgets.QSpinBox()
        self.x_spinbox.setRange(1, 10)
        self.x_spinbox.setValue(self.radius_x)
        self.x_spinbox.valueChanged.connect(self.on_param_changed)
        param_layout.addWidget(self.x_spinbox, 0, 1)
        
        # Y方向半径
        param_layout.addWidget(QtWidgets.QLabel("Y方向半径:"), 1, 0)
        self.y_spinbox = QtWidgets.QSpinBox()
        self.y_spinbox.setRange(1, 10)
        self.y_spinbox.setValue(self.radius_y)
        self.y_spinbox.valueChanged.connect(self.on_param_changed)
        param_layout.addWidget(self.y_spinbox, 1, 1)
        
        # Z方向半径
        param_layout.addWidget(QtWidgets.QLabel("Z方向半径:"), 2, 0)
        self.z_spinbox = QtWidgets.QSpinBox()
        self.z_spinbox.setRange(1, 10)
        self.z_spinbox.setValue(self.radius_z)
    ...
    def on_param_changed(self):
        """参数改变时的处理"""
        self.radius_x = self.x_spinbox.value()
        self.radius_y = self.y_spinbox.value()
        self.radius_z = self.z_spinbox.value()
