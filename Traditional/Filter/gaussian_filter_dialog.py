...
class GaussianFilterDialog(QtWidgets.QDialog):
    """高斯滤波对话框，用于预览和配置高斯滤波参数"""
    ...
    def init_ui(self):
        ...
        # 参数设置区域
        param_group = QtWidgets.QGroupBox("高斯滤波参数")
        param_layout = QtWidgets.QGridLayout(param_group)
        
        # X方向标准差
        param_layout.addWidget(QtWidgets.QLabel("X方向标准差:"), 0, 0)
        self.x_spinbox = QtWidgets.QDoubleSpinBox()
        self.x_spinbox.setRange(0.1, 10.0)
        self.x_spinbox.setSingleStep(0.1)
        self.x_spinbox.setDecimals(1)
        self.x_spinbox.setValue(self.sigma_x)
        self.x_spinbox.valueChanged.connect(self.on_param_changed)
        param_layout.addWidget(self.x_spinbox, 0, 1)
        
        # Y方向标准差
        param_layout.addWidget(QtWidgets.QLabel("Y方向标准差:"), 1, 0)
        self.y_spinbox = QtWidgets.QDoubleSpinBox()
        self.y_spinbox.setRange(0.1, 10.0)
        self.y_spinbox.setSingleStep(0.1)
        self.y_spinbox.setDecimals(1)
        self.y_spinbox.setValue(self.sigma_y)
        self.y_spinbox.valueChanged.connect(self.on_param_changed)
        param_layout.addWidget(self.y_spinbox, 1, 1)
        
        # Z方向标准差
        param_layout.addWidget(QtWidgets.QLabel("Z方向标准差:"), 2, 0)
        self.z_spinbox = QtWidgets.QDoubleSpinBox()
        self.z_spinbox.setRange(0.1, 10.0)
        self.z_spinbox.setSingleStep(0.1)
        self.z_spinbox.setDecimals(1)
        self.z_spinbox.setValue(self.sigma_z)
        self.z_spinbox.valueChanged.connect(self.on_param_changed)
        param_layout.addWidget(self.z_spinbox, 2, 1)
    ...
    def on_param_changed(self):
        """参数改变时的处理"""
        self.sigma_x = self.x_spinbox.value()
        self.sigma_y = self.y_spinbox.value()
        self.sigma_z = self.z_spinbox.value()
    ...
    def apply_filter(self):
        ...
        try:
            ...
            # 设置高斯滤波器参数
            variance = [self.sigma_x * self.sigma_x, self.sigma_y * self.sigma_y, self.sigma_z * self.sigma_z]
            gaussian_filter.SetVariance(variance)
            gaussian_filter.SetMaximumKernelWidth(32)  # 设置最大卷积核宽度
            gaussian_filter.SetUseImageSpacing(True if self.spacing else False)
