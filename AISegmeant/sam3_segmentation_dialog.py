...
class Sam3SegmentationDialog(QtWidgets.QDialog):
    """SAM3预分割参数对话框"""
    ...
    def __init__(self, parent=None, current_data=None):
        ...
        self.setWindowTitle("人工智能分割 - SAM3预分割")
        ...
        param_group = QtWidgets.QGroupBox("SAM3 预分割参数")
        ...
        if current_data is not None:
            ...
            self.use_current_radio.setToolTip("使用主界面中当前已加载的CT体数据")
        else:
        ...
        row += 1
        param_layout.addWidget(QtWidgets.QLabel("SAM3模型权重 (*.pth/*.pt):"), row, 0)
        self.checkpoint_edit = QtWidgets.QLineEdit()
        self.checkpoint_edit.setPlaceholderText("选择SAM3模型权重文件...")
        ...
        row += 1
        param_layout.addWidget(QtWidgets.QLabel("模型类型:"), row, 0)
        self.model_type_combo = QtWidgets.QComboBox()
        self.model_type_combo.addItems(["vit_b", "vit_l", "vit_h"])
        self.model_type_combo.setCurrentText("vit_b")
        self.model_type_combo.setToolTip("与权重匹配的SAM编码器类型")
        param_layout.addWidget(self.model_type_combo, row, 1)
        ...
        self.points_per_side_spin.setToolTip("自动掩膜采样密度，越大越细致但更慢")
        ...
        self.pred_iou_spin.setValue(0.86)
        self.pred_iou_spin.setToolTip("候选掩膜质量阈值，越高越严格")
        ...
        self.stability_spin.setValue(0.92)
        self.stability_spin.setToolTip("掩膜稳定性阈值，越高越保守")
        ...
        self.min_area_spin.setToolTip("最小区域面积（像素），用于过滤噪声")
        param_layout.addWidget(self.min_area_spin, row, 1)

        row += 1
        info_label = QtWidgets.QLabel(
            "说明:\n"
            "• 当前实现按切片执行SAM3自动掩膜，再融合为3D二值体。\n"
            "• 该结果定位为预分割，可作为后续编辑与精修的初稿。\n"
            "• 需先安装 segment-anything、torch、opencv-python。"
        )
        ...
        self.run_button = QtWidgets.QPushButton("开始预分割")
    ...
    def validate_and_accept(self):
        ...
        if not checkpoint_path:
            QtWidgets.QMessageBox.warning(self, "输入错误", "请选择SAM3模型权重文件。")
            return
